...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func setupScoreLabel(){
        ...
        bestItemScoreLabelNode.zPosition = 100  //いちばん手前に表示する
    ...
    func didBegin(_ contact: SKPhysicsContact){
        ...
        if(contact.bodyA.categoryBitMask & scoreCategory) == scoreCategory || (contact.bodyB.categoryBitMask & scoreCategory) == scoreCategory{
            ...
            //アイテムと衝突した
            //アイテム取得音
            let music = SKAction.playSoundFileNamed("itemgetsea", waitForCompletion: true)
            self.run(music)
            
            //自身を取り除くアクションを作成
            let removeItem = SKAction.removeFromParent()
            //りんごを消す
            self.deleteItem.run(removeItem)
    ...
    func setupGround(){
        ...
        _ = SKSpriteNode(texture: groundTexture)
    ...
    func setupWall() {
        //壁の画像を読み込む
        let wallTextuer = SKTexture(imageNamed: "wall")
        wallTextuer.filteringMode = .linear
        
        //移動する距離を計算
        let movingDistance = CGFloat(self.frame.size.width + wallTextuer.size().width)
        ...
        //鳥が通り抜ける隙間の長さを鳥の3倍にする
        let slit_length = birdSize.height * 3
        
        //隙間位置の上下の振れ幅を鳥のサイズの三倍にする
        let random_y_range = birdSize.height * 3
        
        //下の壁のY軸下限位置（中央位置から下方向の最大振れ幅で下の壁を表示する位置）を計算
        let groundSize = SKTexture(imageNamed: "ground").size()
        let center_y = groundSize.height + (self.frame.size.height - groundSize.height) / 2
        let under_wall_lowest_y = center_y - slit_length / 2 - wallTextuer.size().height / 2 - random_y_range / 2
        
        //壁を生成するアクションを作成
        let createWallAnimation = SKAction.run({
            //壁関連のノードを乗せるノードを作成
            let wall = SKNode()
            wall.position = CGPoint(x: self.frame.size.width + wallTextuer.size().width / 2, y: 0)
            ...
            //下側の壁を作成
            let under = SKSpriteNode(texture: wallTextuer)
            under.position = CGPoint(x: 0, y: under_wall_y)
            
            //スプライトに物理演算を設定する
            under.physicsBody = SKPhysicsBody(rectangleOf: wallTextuer.size())
            ...
            //上側の壁を作成
            let upper = SKSpriteNode(texture: wallTextuer)
            upper.position = CGPoint(x: 0, y: under_wall_y + wallTextuer.size().height + slit_length)
            
            //スプライトに物理演算を設定する
            upper.physicsBody = SKPhysicsBody(rectangleOf: wallTextuer.size())
    ...
    func setupItem(){
        ...
        //移動する距離を計算
        let movingDistace = CGFloat(self.frame.size.width + itemTexture.size().width)
        
        //画面外まで移動するアクションを生成
        let moveItem = SKAction.moveBy(x: -movingDistace, y: 0, duration: 4)
        
        //自信を取り除くアクションを生成
        let removeItem = SKAction.removeFromParent()
        
        //二つのアニメーションを順に繰り返すアクションを作成
        ...
        let item_lowest = center_y - slit_length / 2 - itemTexture.size().height / 2 - random_y_range / 2
        ...
        let createItemAnimation = SKAction.run({
            ...
            let under_item_y = item_lowest + random_y
